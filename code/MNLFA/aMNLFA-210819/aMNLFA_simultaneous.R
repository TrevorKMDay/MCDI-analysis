...
aMNLFA.simultaneous <- function(input.object, keepmean = FALSE) {
  ...
  if (thresholds == TRUE) {
    stop(
      "thresholds == TRUE is disabled in this version of aMNLFA. Reset thresholds to FALSE to run this function."
    )
  }
  ...
  semicolon <- paste(";")
  AUXILIARY <-
    ifelse(length(myauxiliary) > 0, paste("AUXILIARY="), paste("!"))
  ...
  meanimpact <-
    MplusAutomation::readModels(fixPath(file.path(dir, "meanimpactscript.out", sep =
                                                    "")))
  ...
  for (j in 1:length(keepmeanimpact)) {
    if (length(grep("_", keepmeanimpact[j])) > 0)
      keepmeanimpact <-
        append(keepmeanimpact, substr(keepmeanimpact[j], 1, 3))
    if (length(grep("_", keepmeanimpact[j])) > 0)
      keepmeanimpact <-
        append(keepmeanimpact, substr(keepmeanimpact[j], 5, 7))
  }
  ...
  varimpact <-
    MplusAutomation::readModels(fixPath(file.path(dir, "varimpactscript.out", sep =
                                                    "")))
  varimpact <- as.data.frame(varimpact$parameters$unstandardized)
  
  varimpact <-
    varimpact[which(varimpact$paramHeader == "New.Additional.Parameters" &
                      varimpact$pval < .1), ]  ######alpha <.1 to trim###########
  varimpact <- noquote(substr(varimpact$param, 2, 3))
  myVarImpact2 <- as.data.frame(myVarImpact)
  myVarImpact3 <- as.data.frame(NULL)
  
  if (!is.null(dim(varimpact))) {
    #added by IS to accommodate no var impact
  ...
  alllambdadf <-
    data.frame(matrix(
      c(NA),
      nrow = length(myindicators),
      ncol = 1 + length(myMeasInvar)
    ))
  colnames(alllambdadf) = c("items", myMeasInvar)
  alllambdadf$items = myindicators
  for (i in 1:length(myindicators)) {
    dif <-
      MplusAutomation::readModels(fixPath(file.path(dir, base::tolower(
        paste("measinvarscript_", myindicators[i], ".out", sep = "")
      ))))
    dif <- dif$parameters$unstandardized
    
    lambdadif <-
      dif[which(dif$paramHeader == "New.Additional.Parameters"), ]
    
    #Addition, 6/15: This gets rid of threshold parameters if they are present
    lambdadif <-
      lambdadif[(grep("L", lambdadif$param)), ] 
    lambdadif$param <-
      noquote(stringr::str_sub(lambdadif$param, -2, -1))
    # 
    # print(i)
    # print("lambdadif")
    # print(lambdadif)
    # print("param")
    # print(lambdadif$param)
    # print("param != 00")
    # print(lambdadif$param != "00")
    ...
    #New as of 6/5
    lambdadif$param <-
      noquote(stringr::str_sub(lambdadif$param, 2, length(lambdadif$param)))
    ...
    for (v in 1:length(myMeasInvar)) {
      lambda_min[v] <-
        (as.numeric(min(
          subset(lambdadif, colnames(lambdadif) == myMeasInvar[v])
        )))
    }
    #Fix indexing issue with threshold DIF
    lambda_index <-
      length(myMeasInvar) #Gets the number of covariates there are, regardless of whether we're using thresholds or not.
    alllambdadf[i, 2:(lambda_index + 1)] <- lambda_min
    for (j in 1:length(myMeasInvar)) {
      alllambdadf[i, j + 1] <-
        ifelse(alllambdadf[i, j + 1] < .05, myMeasInvar[j], NA)
    }
    ...
    for (j in 1:length(keeplambda)) {
      if (length(grep("_", keeplambda[j])) > 0)
        keeplambda <-
          append(keeplambda, stringr::str_sub(keeplambda[j], 1, 3))
      if (length(grep("_", keeplambda[j])) > 0)
        keeplambda <-
          append(keeplambda, stringr::str_sub(keeplambda[j], 5, 7))
    }
    ...
    for (j in 2:(lambda_index + 1)) {
      if (length(keeplambda) > 0) {
        for (k in 1:length(keeplambda)) {
          if (length(grep(keeplambda[k], myMeasInvar[j - 1])) > 0)
            alllambdadf[i, j] <- myMeasInvar[j - 1]
        }
      }
    }
  }
  ...
  if (thresholds == FALSE) {
    ...
    for (i in 1:length(myindicators)) {
      ...
      dif <- dif$parameters$unstandardized
      keep <-
        c("param", "pval") #headers we'll keep -- change for threshold
      intdif <-
        dif[grep(".ON", dif$paramHeader), ] #subset to just mean effects for each item (don't have to worry about ETA here)
      intdif <-
        noquote(t(intdif[keep])) #gives dataframe with number of rows = 2 ("param" and "pval") and number of columns = number of DIF covariates
      ...
      allinterceptdf[i, 2:(length(intdif) + 1)] <-
        intdif #now add the p. values to allinterceptdf dataframe
      for (j in 1:length(myMeasInvar)) {
        #now set the i,j^th value of the matrix to the name of the covariate if the corresponding p. value is < .05, NA otherwise
        allinterceptdf[i, j + 1] <-
          ifelse(allinterceptdf[i, j + 1] < .05, myMeasInvar[j], NA)
      }
      keepintercept <-
        unlist(as.list(allinterceptdf[i, (1:length(myMeasInvar)) + 1]))
      keepintercept <- keepintercept[!is.na(keepintercept)]
      names(keepintercept) <- NULL
      for (j in 1:length(keepintercept)) {
        if (length(grep("_", keepintercept[j])) > 0)
          keepintercept <- append(keepintercept, substr(keepintercept[j], 1, 3))
        if (length(grep("_", keepintercept[j])) > 0)
          keepintercept <- append(keepintercept, substr(keepintercept[j], 5, 7))
      }
      ...
      if (length(keepintercept) > 0)
        for (j in (1:length(myMeasInvar)) + 1) {
          for (k in 1:length(keepintercept)) {
            if (length(grep(keepintercept[k], myMeasInvar[j - 1])) > 0)
              allinterceptdf[i, j] <- myMeasInvar[j - 1]
          }
        }
    }
  } else {
    ...
    for (i in 1:length(myindicators)) {
      ...
      keep <-
        c("param", "pval") #headers we'll keep -- change for threshold
      thrdif <-
        dif[-grep("L|00", dif$param), ] #combine the param condition (don't contain L or 00) to get thresholds
      thrdif <-
        subset(thrdif,
               thrdif$paramHeader == "New.Additional.Parameters")      #...with the paramHeader condition ("New.Additional.Parameters")
      thrdif <-
        noquote(t(thrdif[keep])) #gives dataframe with number of rows = 2 ("param" and "pval") and number of columns = number of DIF covariates
      how.many.thresholds  <-
        sub("\\_.*", "", thrdif[1, ]) #Two-step process to get just the number of threshold -- step 1: extract portion before "_"
      how.many.thresholds  <-
        max(as.numeric(gsub("T", "", how.many.thresholds))) #Step 2: just get the numeric portion of it, and get the maximum
      ...
      allinterceptdf[i, 2:(how.many.thresholds + 1)] <-
        thrdif #now add the p. values to allinterceptdf dataframe
      for (j in 1:length(myMeasInvar)) {
        #now set the i,j^th value of the matrix to the name of the covariate if ANY thresholds have p < .05, NA otherwise
        single.cov  <-  thrdif[grep(paste0("_", j), names(thrdif))]
        allinterceptdf[i, j + 1] <-
          ifelse(sum(single.cov < .05) > 0, myMeasInvar[j], NA)
      }
      keepintercept <-
        unlist(as.list(allinterceptdf[i, (1:length(myMeasInvar)) + 1]))
      keepintercept <- keepintercept[!is.na(keepintercept)]
  ...
  for (i in 1:dim(allinterceptdf)[1]) {
    for (j in 2:dim(allinterceptdf)[2]) {
      allinterceptdf[i, j] <-
        ifelse(is.na(allinterceptdf[i, j]), alllambdadf[i, j], allinterceptdf[i, j])
    }
  }
  ...
  for (q in 1:length(myMeasInvar)) {
    if ((myMeasInvar[q] %in% allinterceptdf[, q + 1]) == TRUE)
      #If there was intercept DIF for covariate q, it will appear in the q+1 column
    {
  ...
  if (keepmean == TRUE) {
    keepmeanimpact <-
      unique(c(keepmeanimpact, intdiflist, keeplambda, keepvarimpact))
  } else {
  ...
  if (length(keepvarimpact) > 0) {
    varindices <-
      which(myVarImpact %in% keepvarimpact) #6/15 change to replace variance parameter names with indices associated with variable order in myVarImpact
    for (v in 1:length(keepvarimpact)) {
      vstart[v, 1] <- paste("v", varindices[v], "*0", sep = "")
    }
  }
  vstart <-
    ifelse(length(keepvarimpact) > 0,
           paste(utils::capture.output(cat(noquote(
             unlist(vstart)
           ))), sep = ""),
           "!")
  new.constraint.section[new.constraint.row, 1] <-
    utils::capture.output(cat(append(MODELCONSTRAINT, vstart)))
  
  
  constraint.section[constraint.row, 1] <- "veta=1*exp("
  if (length(keepvarimpact) > 0)
    for (v in 1:length(keepvarimpact)) {
      constraint.section[constraint.row + 1, 1] <-
        paste("v", varindices[v], "*", keepvarimpact[v], "+", sep = "")
  ...
  for (i in 1:ind) {
    ...
    predlist2 <-
      unlist(alllambdadf[i, 1:m + 1]) #IS changed to 1:m+1....was omitting first column of lambda DIF before! :(
    predlist2 <- predlist2[!is.na(predlist2)]
    eq <- as.data.frame(NULL)
    start <- as.data.frame(NULL)
    eq[1, 1] <-
      ifelse(length(predlist2) > 0,
             paste("l_", i, "=l", i, "_0", sep = ""),
             "!")
    start[1, 1] <-
      ifelse(length(predlist2) > 0, paste("l", i, "_0*1 ", sep = ""), "!")
    if (length(predlist2) > 0)
      predlist2indices <-
      which(myMeasInvar %in% predlist2) #6/15 -- Here too, like other parts, give the index that corresponds to its order in myMeasInvar
    for (w in 1:length(predlist2)) {
      eq[1 + w, 1] <-
        ifelse(
          length(predlist2) > 0,
          paste("+l", i, "_", predlist2indices[w], "*", predlist2[w], sep = "") ,
          "!"
        )
      start[1 + w, 1] <-
        ifelse(length(predlist2) > 0,
               paste(" l", i, "_", predlist2indices[w], "*0", sep = "") ,
               "!")
    }
    constraint.section[constraint.row + 1, 1] <-
      paste(utils::capture.output(cat(noquote(unlist(
        eq
      )))), semicolon, sep = "")
  ...
  if (thresholds == FALSE) {
    ...
    for (i in 1:l) {
      th <-
        length(unique(mrdata[stats::complete.cases(mrdata), myindicators[l]])) -
        1
      sig.covs.thr <- allinterceptdf[i, ]
      cov.index <- seq(0, th)
      if (sum(is.na(sig.covs.thr)) > 0) {
        cov.index <-
          cov.index[is.na(sig.covs.thr) == FALSE] #only choose values of cov.index for which the corresponding value of sig.covs.thr is significant
        ...
        for (k in 1:th) {
          ...
          constraint.section[constraint.row + 2, 1] <-
            paste0("T", i, "_", k, "_0 + ")
          new.constraint.section[new.constraint.row + 1, 1] <-
            paste0("T", i, "_", k, "_0")
          new.constraint.row <- nrow(new.constraint.section)
          for (q in 1:length(sig.covs.thr)) {
            new.constraint.section[new.constraint.row + q, 1] <-
              paste0("T", i, "_", k, "_", cov.index[q])
            constraint.section[constraint.row + q + 2, 1] <-
              paste0(
                "T",
                i,
                "_",
  ...
  round2input[10, 1] <-
    paste("USEVARIABLES= ", useround2, semicolon, sep = "")
  round2input[11, 1] <- AUXILIARY
  round2input[12, 1] <-
    ifelse(length(mycatindicators) > 0, CATEGORICAL, "!")
  round2input[13, 1] <-
    ifelse(length(mycountindicators) > 0, COUNT, "!")
  ...
  round2input[17 + l, 1] <-
    ifelse(length(keepmeanimpact) > 0, utils::capture.output(cat(ETAON2)), "!")
  ...
  if (thresholds == FALSE) {
    for (i in 1:l) {
      ...
      predlist <-
        unlist(allinterceptdf[i, 1:length(myMeasInvar) + 1])  #changed by IS to get all possible intercept DIF --should go thru length(myMeasInvar)+1
      predlist <- predlist[!is.na(predlist)]
      predlist <- utils::capture.output(cat(predlist))
      round2input[the.row + i, 1] <-
        ifelse(length(predlist) > 0,
               paste(myindicators[i], " on ", predlist, ";", sep = ""),
               "!") #VC: check this
      the.row <- nrow(round2input)
    }
  } else {
    for (i in 1:l) {
      #IS not sure if this needs to be or what to do for this section when thresholds=TRUE??
      th <-
        length(unique(mrdata[stats::complete.cases(mrdata), myindicators[l]])) -
        1
      if (sum(is.na(allinterceptdf[i, ])) > 0) {
        for (k in 1:th) {
          round2input[the.row + 1, 1] <-
            paste0("[", myindicators[i], "$", k, "](T", i, "_", k, ");")
          the.row <- nrow(round2input)
        }
      }
    }
  ...
  round2input <-
    rbind(round2input, new.constraint.section, constraint.section)
  the.row <- nrow(round2input)
  ...
  message(
    "COMPLETE. Check '",
    dir,
    "/' for Mplus inp file for round 2 calibration model (run this manually)."
  )
  message(
