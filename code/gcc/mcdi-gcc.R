...
BiocManager::install(version = "3.12") # For R version 4.0.4
library(BiocManager)
BiocManager::install(c("Ruuid", "graph", "Rgraphviz"))
...
p1_graph <- graph_from_data_frame(p1_expand_grid, directed = FALSE)

plot.igraph(p1_graph)

edge_density(p1_graph)
