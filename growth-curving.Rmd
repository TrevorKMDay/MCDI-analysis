...
```{r plot2, echo=FALSE, fig.height=3, fig.width=5}
...
```{r LS plots, echo=FALSE, fig.height=3.5, fig.width=3.5, message=FALSE, warning=FALSE}
...
Although the CFA technically failed (fit parameters marginal), it does seem 
there is a real difference in $T_i$ between syntax and lexical acquisition.
We can compare this difference by randomly selecting identically sized parts,
calculating the average difference and comparing that distribution to the 
average difference between $T_i,syn$ and $T_i,lex$ 
(`r round(mean(LS.vals$synMlex, na.rm = TRUE), 2)`).
...
demo.plot <- lexVsyn(filter(lexsyn, data_id == ID3[3], name != "SUM")) 

demo.plot +
  stat_function(fun = function(.x) 1/(60^2) * .x^2)
